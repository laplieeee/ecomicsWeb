...
$sql = $conn->query("select * from brand where Bra_Name = '$_REQUEST[name]'")or die ;
...
$sql = $conn->query("select * from brand where Bra_Name = '$_REQUEST[name]' && Bra_ID != '$_REQUEST[id]'")or die;
if($sql->num_rows>0){
...
$sql = $conn->query("update brand set Bra_Name = '$_REQUEST[name]' where Bra_ID = '$_REQUEST[id]'")or die ;
...
$sql = $conn->query("delete from brand where Bra_ID = '$_REQUEST[id]'")or die ;
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
									...
									<?php 
									$sql = $conn->query("select * from brand where Bra_ID = '$_REQUEST[id]'")or die ;
...
$sql = $conn->query("select * from brand order by Bra_ID desc limit $start,$perpage")or die ;

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกแสดงเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from brand order by Bra_ID desc")or die ;
...
$sql = $conn->query("select * from brand where Bra_Name like '%$_REQUEST[txt_search]%' order by Bra_ID desc limit $start,$perpage")or die ;

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกค้นหาเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from brand where Bra_Name like '%$_REQUEST[txt_search]%' order by Bra_ID desc")or die ;
$total_record = $sql2->num_rows;
$total_page = ceil($total_record / $perpage);
		}
