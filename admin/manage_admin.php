...
$sql = $conn->query("select * from member where Mem_User = '$_REQUEST[user]'")or die;
...
$sql = $conn->query("select * from member where Mem_User = '$_REQUEST[user]' && Mem_ID != '$_REQUEST[id]'")or die ;
...
$sql = $conn->query("update member set Mem_Name = '$_REQUEST[name]',Mem_User = '$_REQUEST[user]',Mem_Pass = '$_REQUEST[pass]',Mem_Email = '$_REQUEST[email]' where Mem_ID = '$_REQUEST[id]'")or die ;
...
$sql = $conn->query("delete from member where Mem_ID = '$_REQUEST[id]'")or die ;
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
									...
									<?php 
									$sql = $conn->query("select * from member where Mem_ID = '$_REQUEST[id]'")or die ;
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-12">
									...
									<div class="panel"><a href="?admin=add"><input name="" type="button" class="btn btn-success" value="เพิ่มข้อมูล ++">
                                </a></div>
...
$sql = $conn->query("select * from member where Mem_Status = 1 order by Mem_ID desc limit $start,$perpage")or die ;

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกแสดงเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from member where Mem_Status = 1 order by Mem_ID desc")or die ;
...
$sql = $conn->query("select * from member where Mem_Status = 1 && Mem_Name like '%$_REQUEST[txt_search]%' or Mem_User like '%$_REQUEST[txt_search]%' or Mem_Email like '%$_REQUEST[txt_search]%' or Mem_Tel like '%$_REQUEST[txt_search]%' order by Mem_ID desc limit $start,$perpage")or die ;

//หาจำนวน row ทั้งหมด ของ ข้อมูลที่ถูกค้นหาเพื่อจะเอาไปทำการแบ่งหน้า
$sql2 = $conn->query("select * from member where Mem_Status = 1 && Mem_Name like '%$_REQUEST[txt_search]%' or Mem_User like '%$_REQUEST[txt_search]%' or Mem_Email like '%$_REQUEST[txt_search]%' or Mem_Tel like '%$_REQUEST[txt_search]%' order by Mem_ID desc")or die ;
$total_record = $sql2->num_rows;
$total_page = ceil($total_record / $perpage);
		}
...
?>
				  ...
				  <td><div align="center"><a href="?admin=edit&id=<?php echo $show['Mem_ID'];?>"><input name="" type="button" 
                  class="btn btn-primary" value="Edit"></a>&nbsp;<a href="?admin=delete&id=<?php echo $show['Mem_ID'];?>"><input 
                  name="" type="button" class="btn btn-danger" value="Delete"></a></div></td>
                  </tr>
				  <?php }?>
